...
---@class prefyl.compiler.Chunk
---@field package inner string
---@field package output string?
---@field package inputs prefyl.compiler.Chunk[]
---@field package fixed boolean
local M = {}
---@private
M.__index = M

---@param inputs prefyl.compiler.Chunk[]
---@return string[]
local function get_conflicted_outputs(inputs)
    local inputs_map = {} ---@type table<string, prefyl.compiler.Chunk>
...
local function flatten_inputs(inputs)
    ---@type prefyl.compiler.Chunk[]
    local inputs = vim.iter(inputs)
        :map(function(input) ---@param input prefyl.compiler.Chunk
...
local function is_fixed(default, output, inputs)
    if
        vim.iter(inputs):any(function(input) ---@param input prefyl.compiler.Chunk
            return input.fixed
        end)
    then
        return true
...
---@param chunk string
---@param opts { output: string?, inputs: prefyl.compiler.Chunk[]?, fixed: boolean? }?
---@return prefyl.compiler.Chunk
...
---@param input prefyl.compiler.Chunk
...
---@param chunk prefyl.compiler.Chunk
---@param root boolean
---@param conflicted_outputs string[]?
---@return string
local function tostr(chunk, root, conflicted_outputs)
    local s = vim.iter(chunk.inputs)
        :filter(function(input) ---@param input prefyl.compiler.Chunk
        ...
        :join("") .. chunk.inner
    if
        vim.iter(chunk.inputs):any(function(input) ---@param input prefyl.compiler.Chunk
...
local function get_shared_inputs(inputs, conflicted_outputs)
    return vim.iter(inputs)
        ...
        :fold({}, function(acc, input) ---@param input prefyl.compiler.Chunk
            local duplicated = vim.iter(acc):any(function(c) ---@param c prefyl.compiler.Chunk
...
---@param opts { inputs: prefyl.compiler.Chunk[]? }?
---@return prefyl.compiler.Chunk
...
---@param name string
---@param args string[]
---@param fn fun(args: table<string, prefyl.compiler.Chunk>): prefyl.compiler.Chunk
---@param opts { fixed: boolean? }?
---@return prefyl.compiler.Chunk
function M.function_(name, args, fn, opts)
    local opts = opts or {}
    ---@type table<string, prefyl.compiler.Chunk>
...
---@class prefyl.compiler.chunk.Scope: { [integer]: prefyl.compiler.Chunk }
local Scope = {}
---@private
Scope.__index = Scope

---@param default prefyl.compiler.Chunk[]?
---@return prefyl.compiler.chunk.Scope
function M.scope(default)
    if (getmetatable(default) or {}).__index == Scope then
        ---@cast default prefyl.compiler.chunk.Scope
        return default
    end
    return setmetatable(default or {}, Scope)
end

---@param chunk prefyl.compiler.Chunk | string
---@return self
...
---@param chunks prefyl.compiler.Chunk[]
---@return self
...
function Scope:to_chunk()
    ...
    return M.new(
        vim.iter(self)
            :map(function(chunk) ---@param chunk prefyl.compiler.Chunk
